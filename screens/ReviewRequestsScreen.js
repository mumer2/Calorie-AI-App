...
export default function ReviewRequestsScreen() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (response.ok) {
        ...
        Alert.alert(i18n.t('error'), data.message || i18n.t('fetchRequestsFailed'));
      }
    } catch (err) {
  ...
  const sendReply = async (item) => {
    if (!item.tempReply || item.tempReply.trim() === '') {
      return Alert.alert(i18n.t('error'), i18n.t('emptyReply'));
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        Alert.alert(i18n.t('error'), data.message || i18n.t('replyFailed'));
      }
    } catch (error) {
      Alert.alert(i18n.t('error'), i18n.t('networkError'));
    } finally {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>📥 {i18n.t('memberRequests')}</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          ...
          >
            <Text style={styles.name}>{i18n.t('from')}: {item.userName}</Text>
            <Text style={styles.message}>{item.message}</Text>
            <Text style={styles.timestamp}>
              {i18n.t('sentAt')}: {new Date(item.timestamp).toLocaleString()}
            </Text>

            {item.reply ? (
              <>
                <Text style={styles.replyLabel}>{i18n.t('coachReply')}:</Text>
                <Text style={styles.replyText}>{item.reply}</Text>
              </>
            ) : (
              <View style={{ marginTop: 10 }}>
                <Text style={styles.replyLabel}>{i18n.t('yourReply')}:</Text>
                <TextInput
                  style={styles.replyInput}
                  placeholder={i18n.t('replyPlaceholder')}
                  multiline
                  onChangeText={(text) => (item.tempReply = text)}
                />
                <TouchableOpacity
                  style={[
                    styles.replyButton,
                ...
                >
                  {sendingReplyId === item._id ? (
                    ...
                    <Text style={styles.replyButtonText}>{i18n.t('sendReply')}</Text>
                  )}
                </TouchableOpacity>
              </View>
            )}
        ...
        ListEmptyComponent={<Text style={styles.empty}>{i18n.t('noRequests')}</Text>}
      />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 22, fontWeight: 'bold', color: '#0e4d92', marginBottom: 16, textAlign: 'center' },
  card: {
    backgroundColor: '#fff',
    padding: 16,
    marginBottom: 12,
  ...
  name: { fontWeight: 'bold', marginBottom: 6, color: '#333' },
  message: { color: '#444', marginBottom: 6 },
  timestamp: { fontSize: 12, color: '#777', marginBottom: 6 },
  replyLabel: { marginTop: 8, fontWeight: 'bold', color: '#0e4d92' },
  replyText: {
    marginTop: 4,
